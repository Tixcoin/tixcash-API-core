...
func UnmarshalFixedAddressJSON(typ reflect.Type, input, out []byte) error {
	if !isString(input) {
		return errNonString(typ)
	}
	return wrapTypeError(UnmarshalAddress(typ.String(), input[1:len(input)-1], out), typ)
}
...
func UnmarshalAddress(typname string, input, out []byte) error {
	raw, err := checkAddress(input, true)
	if err != nil {
		return err
	}
	if len(raw)/2 != len(out) {
...
func bytesHaveGPrefix(input []byte) bool {
	return len(input) >= 1 && (input[0] == 'g' || input[0] == 'G')
}
...
func checkAddress(input []byte, wantPrefix bool) ([]byte, error) {
	if len(input) == 0 {
		return nil, nil // empty strings are allowed
	}
	if bytesHaveGPrefix(input) {
		input = input[1:]
	} else if wantPrefix {
		return nil, ErrMissingPrefix
	}
