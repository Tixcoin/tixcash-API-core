...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"tixcash": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	message := "Welcome to the TixCash JavaScript console!\n\n"

	// Print some generic Geth metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + tixcash.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + tixcash.blockNumber + " (" + new Date(1000 * tixcash.getBlock(tixcash.blockNumber).timestamp) + ")\n";
		try {
